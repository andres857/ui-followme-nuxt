    ...
    <transition enter-active-class="ease-out duration-300" enter-class="opacity-0" enter-to-class="opacity-100" leave-active-class="ease-in duration-200 " leave-class="opacity-100" leave-to-class="opacity-0 ">
        ...
        <div class="relative z-10 modal-container" aria-labelledby="modal-title" role="dialog" aria-modal="true" >
            ...
            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div v-if="editModalOpen" :item="selectedItem" @save="handleSave" @close="closeEditModal" class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    <div name="editFloor"  class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg" >
                ...
                <transition enter-active-class="ease-out duration-300" enter-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to-class="opacity-100 translate-y-0 sm:scale-100" leave-active-class="ease-in duration-200 " leave-class="opacity-100 translate-y-0 sm:scale-100" leave-to-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <Success v-if="successModalOpen" @close="closeSuccessModal"></Success>
                </transition>

                <transition enter-active-class="ease-out duration-300" enter-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to-class="opacity-100 translate-y-0 sm:scale-100" leave-active-class="ease-in duration-200 " leave-class="opacity-100 translate-y-0 sm:scale-100" leave-to-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <Failed v-if="errorModalOpen" @close="closeErrorModal" ></Failed>
                </transition>

                <transition enter-active-class="ease-out duration-300" enter-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to-class="opacity-100 translate-y-0 sm:scale-100" leave-active-class="ease-in duration-200 " leave-class="opacity-100 translate-y-0 sm:scale-100" leave-to-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <Delete v-if="deleteConfirmationModalOpen" @confirm="handleDelete" @cancel="closeDeleteConfirmationModal" ></Delete>
                </transition>

                <transition enter-active-class="ease-out duration-300" enter-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to-class="opacity-100 translate-y-0 sm:scale-100" leave-active-class="ease-in duration-200 " leave-class="opacity-100 translate-y-0 sm:scale-100" leave-to-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    <Processing v-if="loadingModalOpen" ></Processing>
                </transition>
...
export default {
  ...
  methods: {
    openEditFloor(item) {
      ...
      this.EditFloorOpen = true
    },
    closeEditFloor() {
      this.EditFloorOpen = false
    },
    handleSave(updatedItem) {
      this.loadingModalOpen = true
      // Aquí harías la llamada a la API para actualizar el item
      // y luego actualizarías el estado de los modales
      this.loadingModalOpen = false
      if (/* la actualización fue exitosa */) {
    ...
    handleDelete() {
      this.loadingModalOpen = true
      // Aquí harías la llamada a la API para eliminar el item
      // y luego actualizarías el estado de los modales
      this.loadingModalOpen = false
      this.successModalOpen = true
      this.deleteConfirmationModalOpen = false
